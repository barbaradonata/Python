...
def criar_interface():
    ...
    saldo_label = tk.Label(root, text="Saldo: R$ 0.00", width=50)
    saldo_label.grid(row=0, column=0, columnspan=2)

    valor_entry = tk.Entry(root, width=50)
    valor_entry.grid(row=1, column=0, columnspan=2)

    btn_depositar = tk.Button(root, text="Depositar", command=lambda: depositar(saldo_label, valor_entry,  width=10))
    btn_depositar.grid(row=2, column=0)

    btn_sacar = tk.Button(root, text="Sacar", command=lambda: sacar(saldo_label, valor_entry,  width=10))
